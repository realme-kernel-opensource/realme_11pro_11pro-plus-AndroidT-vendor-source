...
extern void sc8517_set_fixmode(bool val);
...
static void oplus_chg_print_log(struct oplus_chg_chip *chip)
{
	...
	if (oplus_chg_get_gsm_call_on() == true) {
		chg_err("(%d, %d)\n", callon_pre, chip->gsm_call_ongoing);
		if ((chip->gsm_call_ongoing && !callon_pre) || (!chip->gsm_call_ongoing && callon_pre)) {
			sc8517_set_fixmode(chip->gsm_call_ongoing);
		} else {
			chg_err("stay in the mode\n");
		}
		callon_pre = chip->gsm_call_ongoing;
		chg_err("set/out fixmode done!!\n");
	}
#endif
}
