...
void sc8517_set_fixmode( bool val)
{
	int ret = 0;
	u8 val_fix = 0;
	if (!oplus_voocphy_mg) {
		...
		if (val) {
			sc8517_write_byte(oplus_voocphy_mg->client, SC8517_REG_07, 0x46);//enable fixmode when at GSM
		} else {
			sc8517_write_byte(oplus_voocphy_mg->client, SC8517_REG_07, 0x06);//disable fixmode when out GSM
		}
	}
	ret = sc8517_read_byte(oplus_voocphy_mg->client, SC8517_REG_07, &val_fix);
	if (ret < 0) {
		chg_err("SC8517_REG_07\n");
	}
	chg_err("----val_fix = %d\n",val_fix);
}
EXPORT_SYMBOL(sc8517_set_fixmode);
#endif
